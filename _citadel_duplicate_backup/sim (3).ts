...
export class CitySim {
  ...
  snapshot(): Snapshot {
    return {
      ...
      vehicles: this.vehicles.filter((v) => v.emergency).map((v) => ({ ...v, missionEvents: v.missionEvents?.map((e) => ({ ...e })) })),
      ...
      dependencyAnalysis: this.dependencyTrace ? {
        ...
        requirements: this.dependencyTrace.requirements.map((x) => ({ ...x })),
    ...
    this.algorithmTrace = [
      {
        ...
        output: `${analysis.requirements.length} operational requirements / ${analysis.direct.length} direct mapped / ${analysis.indirect.length} indirect / ${analysis.hiddenRelations.length} hidden`,
  ...
  dispatch(kind: Vehicle['kind'], incident: Incident, targetNode: number) {
    ...
    const [originLon, originLat] = toLonLat(unit.x, unit.y);
    unit.path = fallbackPath;
    unit.leg = 0;
    unit.t = 0;
    unit.status = 'RESPONDING';
    unit.targetIncident = incident.id;
    unit.missionTargetNode = targetNode;
    unit.missionStartedAt = this.simSeconds;
    unit.missionOriginLabel = `${unit.id} BASE`;
    unit.missionOriginX = unit.x;
    unit.missionOriginY = unit.y;
    unit.missionOriginLat = originLat;
    unit.missionOriginLon = originLon;
    unit.missionDestinationLabel = `${incident.kind} // ${incident.edgeId}`;
    unit.missionDestinationX = incident.x;
    unit.missionDestinationY = incident.y;
    unit.currentCongestionPct = 0;
    unit.peakCongestionPct = 0;
    unit.rerouteCount = 0;
    unit.lastRerouteReason = undefined;
    unit.lastRerouteAt = -9999;
    unit.routeRevision = 1;
    unit.missionEvents = [];
    ...
    unit.routeDistanceMeters = dist;
    unit.etaSeconds = dist / Math.max(1, unit.speed * this.speedFactor());
    this.pushMissionEvent(unit, 'DISPATCH', 'UNIT DISPATCHED', `${unit.missionOriginLabel} → ${unit.missionDestinationLabel}`, 'INFO');
    this.pushMissionEvent(unit, 'ROUTE', 'INITIAL ROUTE', `Dynamic Dijkstra // ${fallbackPath.length} nodes // ETA ${etaString(unit.etaSeconds)}`, 'INFO');
  ...
  private async attachTrafficAwareRoute(unit: Vehicle, targetNode: number, phase: 'RESPONDING' | 'RETURNING') {
    ...
    unit.status = phase;
    unit.routeRevision = (unit.routeRevision ?? 0) + 1;
    this.pushMissionEvent(unit, 'ROUTE', 'LIVE TRAFFIC ROUTE', `${(route.lengthMeters / 1000).toFixed(1)} KM // traffic delay ${Math.round(route.trafficDelaySeconds / 60)} min`, route.trafficDelaySeconds > 180 ? 'WARN' : 'INFO');
  ...
  private pushMissionEvent(
    unit: Vehicle,
    kind: NonNullable<Vehicle['missionEvents']>[number]['kind'],
    label: string,
    detail: string,
    tone: NonNullable<Vehicle['missionEvents']>[number]['tone'] = 'INFO',
  ) {
    const events = unit.missionEvents ?? (unit.missionEvents = []);
    events.unshift({
      id: `${unit.id}-${kind}-${Math.round(this.simSeconds * 10)}-${events.length}`,
      atSecond: this.simSeconds,
      kind,
      label,
      detail,
      tone,
  ...
  private nearestNodeTo(x: number, y: number) {
    let best = nodes[0].id;
  ...
  private congestionForUnit(unit: Vehicle) {
    if (this.dataMode === 'LIVE' && this.liveTraffic.connected && Number.isFinite(unit.geoLat) && Number.isFinite(unit.geoLon) && this.liveTraffic.samples.length) {
      const sample = [...this.liveTraffic.samples].sort((a, b) =>
        haversineKm(unit.geoLat!, unit.geoLon!, a.lat, a.lon) - haversineKm(unit.geoLat!, unit.geoLon!, b.lat, b.lon)
      )[0];
      if (sample?.freeFlowSpeedKmh > 0) return clamp((1 - sample.currentSpeedKmh / sample.freeFlowSpeedKmh) * 100, 0, 100);
    }
    const a = unit.path[unit.leg];
    const b = unit.path[unit.leg + 1];
    const edge = a !== undefined && b !== undefined ? edgeBetween(a, b) : undefined;
    if (!edge) return this.liveTraffic.connected ? this.liveTraffic.congestionPct : (this.traffic === 'GRIDLOCK' ? 85 : this.traffic === 'HEAVY' ? 62 : this.traffic === 'NORMAL' ? 28 : 12);
    const load = edge.load / Math.max(.1, edge.capacity);
    const live = edge.liveFreeFlowSpeedKmh > 0 ? (1 - Math.min(1, edge.liveSpeedRatio)) * 100 : 0;
    return clamp(Math.max(live, (load - .55) * 70), 0, 100);
  }

  private floodThreatForUnit(unit: Vehicle): { threatened: boolean; reason: string } {
    const d = this.disaster;
    if (!d || (d.kind !== 'FLOOD' && d.kind !== 'COMPOUND')) return { threatened: false, reason: '' };
    const dangerous = new Set(d.affectedEdges.filter((key) => (d.floodDepthByEdge[key] ?? 0) >= .28 || (edgeByKey(key)?.capacity ?? 1) < .35));
    if (!dangerous.size) return { threatened: false, reason: '' };

    for (let i = unit.leg; i < unit.path.length - 1; i++) {
      const key = `${Math.min(unit.path[i], unit.path[i + 1])}-${Math.max(unit.path[i], unit.path[i + 1])}`;
      if (dangerous.has(key)) return { threatened: true, reason: `FLOODED ROAD ${edgeByKey(key)?.id ?? key} ENTERED ROUTE` };
    }

    if (unit.geoRoute && unit.geoRoute.length > 1) {
      const start = Math.max(0, unit.geoRouteIndex ?? 0);
      const samplePoints = unit.geoRoute.slice(start, Math.min(unit.geoRoute.length, start + 28));
      for (const key of dangerous) {
        const edge = edgeByKey(key);
        if (!edge) continue;
        const mid = this.edgeMidpoint(edge);
        for (let i = 0; i < samplePoints.length; i += 2) {
          const [x, y] = fromLonLat(samplePoints[i].lon, samplePoints[i].lat);
          if (Math.hypot(x - mid.x, y - mid.y) < 170) return { threatened: true, reason: `LIVE ROUTE APPROACHES FLOODED CORRIDOR ${edge.id}` };
        }
      }
    }
    return { threatened: false, reason: '' };
  }

  private rerouteEmergencyAroundHazard(unit: Vehicle, reason: string) {
    const targetNode = unit.missionTargetNode;
    if (targetNode === undefined) return;
    if (this.simSeconds - (unit.lastRerouteAt ?? -9999) < 8) return;
    const d = this.disaster;
    const banned = new Set<string>();
    if (d) d.affectedEdges.forEach((key) => {
      if ((d.floodDepthByEdge[key] ?? 0) >= .28 || (edgeByKey(key)?.capacity ?? 1) < .35) banned.add(key);
    });
    const start = this.nearestNodeTo(unit.x, unit.y);
    const reroute = findPath(start, targetNode, this.simSeconds, true, banned);
    if (reroute.length < 2) return;
    unit.path = reroute;
    unit.leg = 0;
    unit.t = 0;
    unit.geoRoute = undefined;
    unit.geoRouteIndex = 0;
    unit.geoRouteT = 0;
    unit.routeSource = 'HAZARD-AWARE DIJKSTRA';
    unit.routeDistanceMeters = this.pathLength(reroute);
    unit.etaSeconds = unit.routeDistanceMeters / Math.max(1, unit.speed * Math.max(.45, this.speedFactor()));
    unit.rerouteCount = (unit.rerouteCount ?? 0) + 1;
    unit.lastRerouteReason = reason;
    unit.lastRerouteAt = this.simSeconds;
    unit.routeRevision = (unit.routeRevision ?? 0) + 1;
    const inc = this.incidents.find((x) => x.id === unit.targetIncident);
    if (inc) inc.corridor = [...reroute];
    this.pushMissionEvent(unit, 'REROUTE', `REROUTE #${unit.rerouteCount}`, `${reason} // hazard-aware path selected`, 'ALERT');
    this.log(`${unit.id} REROUTED // ${reason}`, 'ALERT');
    this.notice('EMERGENCY ROUTE CHANGED', [`${unit.id} // HAZARD DETECTED AHEAD`, 'DIJKSTRA RECALCULATED A SAFE CORRIDOR'], 'ALERT');
  }

  private updateEmergencyMissions() {
    for (const unit of this.vehicles) {
      if (!unit.emergency || unit.status !== 'RESPONDING') continue;
      const congestion = this.congestionForUnit(unit);
      unit.currentCongestionPct = congestion;
      unit.peakCongestionPct = Math.max(unit.peakCongestionPct ?? 0, congestion);
      const events = unit.missionEvents ?? [];
      const recentCongestion = events.find((e) => e.kind === 'CONGESTION');
      if (congestion >= 55 && (!recentCongestion || this.simSeconds - recentCongestion.atSecond > 12)) {
        this.pushMissionEvent(unit, 'CONGESTION', 'CONGESTION ENCOUNTERED', `${Math.round(congestion)}% corridor congestion // ETA pressure rising`, congestion >= 75 ? 'ALERT' : 'WARN');
      }
      const threat = this.floodThreatForUnit(unit);
      if (threat.threatened) this.rerouteEmergencyAroundHazard(unit, threat.reason);
    }
  }
  ...
  private arrive(v: Vehicle) {
    ...
    this.pushMissionEvent(v, 'ARRIVAL', 'UNIT ARRIVED', `${inc?.kind ?? 'INCIDENT'} // ${inc?.edgeId ?? 'FIELD'} // peak congestion ${Math.round(v.peakCongestionPct ?? 0)}%`, 'OK');
  ...
  private releaseUnit(v: Vehicle) {
    ...
    if (inc && inc.active) {
      ...
      this.pushMissionEvent(v, 'CLEAR', 'INCIDENT CLEARED', `${inc.edgeId} capacity restored`, 'OK');
    ...
    v.leg = 0;
    v.t = 0;
    this.pushMissionEvent(v, 'RETURN', 'RETURN TO BASE', `${v.id} released from scene`, 'INFO');
