...
export default function LocationSearch({ world, sim }: { world: World | null; sim: CitySim }) {
  ...
  const choose=(row:LocationSearchResult)=>{
    ...
    // Give the search dropdown one frame to close, then perform a very visible
    // camera flight. Facility probing runs just after the camera starts moving.
    window.requestAnimationFrame(() => {
      world.flyToLonLat(row.lon, row.lat, 2200);
      window.setTimeout(() => void sim.probeMapPoint(row.lat, row.lon, 15), 250);
    });
  };
