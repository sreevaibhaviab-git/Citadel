import { assets, edgeBetween, edges, LAYER_KEYS, neighbours, nodes, toLonLat } from './config';
import { fetchFacilitiesAround, fetchLiveFacilities, fetchLiveTraffic, fetchLiveWeather } from './live';
...
export class CitySim {
  ...
  dispatch(kind: Vehicle['kind'], incident: Incident, targetNode: number) {
    ...
    unit.path = findPath(from, targetNode, this.simSeconds, true);
    unit.leg = 0;
    unit.t = 0;
    unit.status = 'RESPONDING';
    unit.targetIncident = incident.id;
    incident.assignedUnit = unit.id;
    incident.corridor = unit.path;
    const dist = this.pathLength(unit.path);
    unit.etaSeconds = dist / (unit.speed * this.speedFactor());
    this.log(`${unit.id} DISPATCHED // ETA ${etaString(unit.etaSeconds)}`, 'UNIT');
  ...
  private releaseUnit(v: Vehicle) {
    ...
    v.targetIncident = undefined;
    v.status = 'RETURNING';
    v.path = findPath(v.path[v.path.length - 1], v.station ?? 0, this.simSeconds);
    v.leg = 0;
    v.t = 0;
